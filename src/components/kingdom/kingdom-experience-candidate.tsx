...
import { KingdomSceneCandidate } from "./kingdom-scene-candidate";
...
function useQualityTier(): "low" | "high" {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setQuality(compact || fewerCores ? "low" : "high");
    };
    update();
...
export function KingdomExperienceCandidate({
  initialOwner,
  initialRepository,
  initialRevision,
    ...
    <main className={rootClassName} data-mode={mode}>
      {webglSupported && contextFailureCount < 2 ? (
        <SceneBoundary onError={() => setContextFailureCount(2)}>
          <div key={sceneKey} className={styles.canvasWrap} aria-hidden="true">
            <Canvas
              dpr={dpr}
