...
const outputDirectory = resolve(
  argumentsMap.get("output") ||
    resolve(repositoryRoot, "public/assets/world/architecture/polyhaven"),
);
...
try {
  ...
  throw new Error(
    "Next.js optional dependency sharp is required to rebuild the architecture atlases",
  );
}
...
const channelDefinitions = {
  ...
  normal: {
    encode: { effort: 6, quality: 90, smartSubsample: true },
  },
  roughness: {
...
console.log(
  `Built ${Object.keys(channelDefinitions).length * Object.keys(manifest.atlas.tiers).length} guarded architecture detail atlases.`,
);
